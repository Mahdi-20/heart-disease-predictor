"""
Heart Disease Prediction Web App - ENHANCED
Interactive Streamlit application with patient history tracking, confidence intervals, and SHAP explainability
Original visualization style + Advanced features
"""
...
def add_prediction_to_history(patient_id, patient_name, patient_data, prediction, confidence, risk_percentage):
    ...
    history.append({
        ...
        'age': int(patient_data[0][0]),
        'max_heart_rate': int(patient_data[0][7]),
        'cholesterol': int(patient_data[0][4])
    })
...
with tab1:
    ...
    with st.spinner("Calculating feature explanations..."):
        # Use smaller background sample for faster computation
        X_train_scaled = scaler.transform(X)
        background_sample = X_train_scaled[np.random.choice(X_train_scaled.shape[0], 50, replace=False)]
...
with tab2:
    ...
    if history:
        ...
        display_cols = ['timestamp', 'patient_id', 'patient_name', 'age', 'cholesterol', 'max_heart_rate', 'risk_percentage', 'prediction']
        ...
        if len(patient_history_data) > 1:
            ...
            with col2:
                st.metric("Cholesterol", f"{int(latest_record['cholesterol'])} mg/dl")
            with col3:
                st.metric("Max Heart Rate", f"{int(latest_record['max_heart_rate'])} bpm")
            with col4:
            ...
            display_cols = ['timestamp', 'risk_percentage', 'prediction', 'age', 'cholesterol', 'max_heart_rate']
